...
- `RasterMask(width: Int, height: Int, pngData: Data)`: EXIF 방향이 반영된 원본 좌상단에 맞는 선형 회색조 PNG. 메모리에서는 PNG를 들고 있지만 `CatalogStore`는 카탈로그 옆 `Masks/<sha256>.png`에 저장하고 JSON에는 `sha256`만 쓴다. 마스크 파일을 먼저 쓴 뒤 카탈로그를 교체하고, 참조가 사라진 해시 이름 PNG만 지운다. 이전의 인라인 `pngData` 카탈로그도 읽으며 다음 저장 때 파일로 옮긴다. 파일이 없거나 해시가 다르면 카탈로그 로드 오류로 처리하고 카탈로그를 덮어쓰지 않는다. 긴 변 최대 1536, 데이터 최대 8 MiB, 실제 디코드 크기가 선언 크기와 일치해야 한다. 검증은 이미지 소비 경계에서 수행하고 오류를 숨기지 않는다.
- `RetouchMode: String, Codable, CaseIterable, Sendable`: heal, clone.
- `RetouchStroke: Identifiable`: id UUID, mode RetouchMode, points [MaskPoint], radius Double(원본 짧은 변 비율), sourceOffset MaskPoint? (clone의 source minus first destination 정규화 이동량), isEnabled Bool=true. 기본 mode heal, points [], radius 0.02, sourceOffset nil. clone은 유한한 sourceOffset 필수이다. heal은 앱이 stroke를 추가할 때 `healingSourceOffset(url:edits:stroke:)`로 주변 패치를 한 번 골라 sourceOffset에 저장하고, 렌더는 저장된 값을 그대로 쓴다. sourceOffset이 없는 이전 heal stroke만 렌더 때 탐색한다.
