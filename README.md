...
3. 사진 보기에서 노출, 색온도·틴트, 대비, 하이라이트·섀도, 채도, 선명도와 RGB 곡선·색상 범위·필름 입자를 조절합니다. 자유 크롭과 수평 보정으로 구도를 정합니다. [고급 보정 안내](docs/advanced-editing.md)를 참고하세요.
4. **부분 보정**에서 피사체·배경을 자동 선택하거나 **영역 추가** 후 브러시로 칠해 해당 부분의 노출·대비를 조절합니다. 지우개, 경계 부드럽게, 여러 영역, 마스크 표시를 지원합니다. **복구**에서는 작은 잡티를 복구하거나 직접 고른 소스를 복제합니다.
...
9. **JPEG 내보내기**에서 실제 압축 미리보기와 파일 크기를 확인하고 현재 사진, 선택한 사진 또는 현재 필터 결과를 저장합니다. 긴 변과 품질을 선택할 수 있고 파일명이 겹치면 접미사를 붙입니다. 여러 장에서는 대표 사진 한 장의 크기를 표시하며, 진행 중에 **중지**하면 처리 중인 한 장까지만 저장합니다. 촬영일·카메라·렌즈·노출 정보는 원본에서 옮기며, 위치(GPS)는 **위치(GPS) 정보 포함**을 켠 경우에만 넣습니다.
...
사진과 보정은 `catalog.json`, 내 폴더 구성은 같은 위치의 `folders.json`, LUT와 표시 이름은 `LUTs` 폴더, 자동 선택 마스크는 `Masks` 폴더에 보관합니다. `Thumbnails` 폴더는 보정한 사진의 썸네일 캐시이며 지워도 다시 만들어집니다. 첫 버전은 개인 라이브러리를 대상으로 하며 수만 장의 대규모 처리 성능은 아직 검증하지 않았습니다. 백업할 때 이 파일·폴더들과 원본 사진을 함께 보관하세요.

RAW 지원은 macOS의 카메라 지원에 따릅니다. 현재 개발 Mac에서 S9의 일반 24MP RW2를 실제 현상·내보내기로 확인했습니다. 모든 펌웨어·촬영 모드의 RW2를 확인한 것은 아닙니다. 내보내기는 SDR sRGB JPEG이며 클라우드 동기화와 Lightroom 카탈로그 호환은 제공하지 않습니다. 자동 선택은 사진에 따라 실패하거나 수동 수정이 필요할 수 있습니다.
