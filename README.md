...
9. **JPEG 내보내기**에서 실제 압축 미리보기와 파일 크기를 확인하고 현재 사진, 선택한 사진 또는 현재 필터 결과를 저장합니다. 긴 변과 품질을 선택할 수 있고 파일명이 겹치면 접미사를 붙입니다. 여러 장에서는 대표 사진 한 장의 크기를 표시합니다. 촬영일·카메라·렌즈·노출 정보는 원본에서 옮기며, 위치(GPS)는 **위치(GPS) 정보 포함**을 켠 경우에만 넣습니다.
...
사진과 보정은 `catalog.json`, 내 폴더 구성은 같은 위치의 `folders.json`, LUT와 표시 이름은 `LUTs` 폴더, 자동 선택 마스크는 `Masks` 폴더에 보관합니다. 첫 버전은 개인 라이브러리를 대상으로 하며 수만 장의 대규모 처리 성능은 아직 검증하지 않았습니다. 백업할 때 이 파일·폴더들과 원본 사진을 함께 보관하세요.
